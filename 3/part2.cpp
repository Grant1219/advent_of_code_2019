...
int main(int argc, char** argv) {
    ...
    Vec2 wire1 = { 0, 0};
    Vec2 wire2 = { 0, 0};
    std::map<Vec2, unsigned int> wire1_points;
    std::map<Vec2, unsigned int> wire2_points;
    unsigned int least_steps = 1410065408;

    unsigned int first_steps = 0;
    unsigned int second_steps = 0;
    ...
    while (first_itr != first_moves.end() && second_itr != second_moves.end()) {
        ...
        if (first_itr->first != 0) {
            for (int i = 0; i < std::abs(first_itr->first); ++i) {
                wire1_points.insert({ Vec2(wire1.first, wire1.second), first_steps });
        ...
        if (second_itr->first != 0) {
            for (int i = 0; i < std::abs(second_itr->first); ++i) {
                ...
                wire2_points.insert({ v, second_steps });

                // check every intersection to find which is the least steps
                auto itr = wire1_points.find(v);
                if (itr != wire1_points.end()) {
                    unsigned int combined_steps = second_steps + itr->second;
                    if (combined_steps < least_steps && combined_steps > 0)
                        least_steps = combined_steps;
                }
    ...
    std::cout << "The least steps to an intersection is: " << least_steps << std::endl;
